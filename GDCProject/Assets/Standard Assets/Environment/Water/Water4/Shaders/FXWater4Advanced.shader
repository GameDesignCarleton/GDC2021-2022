...
Properties {
	...
	_PulseOrigin ("Pulse Origin", Vector) = (0,0, 0)
	_PulseStartTime ("Pulse Start Time", float) = -10
...
CGINCLUDE
	...
	uniform float3 _PulseOrigin;
	uniform float _PulseStartTime;
	...
	float TimedAmplitude(){
		float x = (_Time.y-_PulseStartTime);
		return 1/(1+exp(-1*(x-3)));
	}

	float PulseStrength(float3 worldSpaceVertex){
		float wavelength = 0.1;
		float waveSpeed = 10;
		float pulseDistance = distance(worldSpaceVertex,_PulseOrigin);
		float wavePosition = (pulseDistance-waveSpeed*(_Time.y-_PulseStartTime));
		
		return 250*_PulseAmplitude*TimedAmplitude()*/*exp(-(_Time.y-_PulseStartTime)*0.3f)*/sin(wavelength * wavePosition)/(wavePosition);
	...
	v2f vert(appdata_full v)
	{
		...
		half3 nrml;
		half3 offsets;
		v.vertex.y += PulseStrength(worldSpaceVertex);
